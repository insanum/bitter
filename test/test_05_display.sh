...
#
# EXAMPLES:
#   bitz --signed 0xffffffff        # Show as signed: -1
#   bitz --stats 0xdeadbeef         # Show popcount, leading zeros, etc.
#   bitz --dec 0xff                 # Show decimal: 255
#   bitz --bin 0xff                 # Show binary
#   bitz --width=64 0xff            # Force 64-bit display
#   bitz --compact 0xdeadbeef       # Single-line output
#   bitz --theme=light 0xff         # Use light terminal theme
#   bitz --format="%h (%d)" 0xff    # Custom format
#
...
run_test "Signed - positive 32-bit" \
    "$BITZ -n --signed 0x7fffffff" \
    "signed: 2147483647"

run_test "Signed - negative 32-bit (-1)" \
    "$BITZ -n --signed 0xffffffff" \
    "signed: -1"

run_test "Signed - negative 32-bit (min)" \
    "$BITZ -n --signed 0x80000000" \
    "signed: -2147483648"
...
run_test "Signed - 64-bit negative" \
    "$BITZ -n --signed 0xffffffffffffffff" \
    "signed: -1"
...
run_test "Multiple bases - dec and oct" \
    "$BITZ -n --dec --oct 0xff | grep -c ':'" \
    "2"

run_test "Multiple bases - all formats" \
    "$BITZ -n --dec --oct --bin --signed 0xff | grep -c ':'" \
    "4"
...
run_test "Width - force 32-bit (default)" \
    "$BITZ -n --width=32 0xff" \
    "0x000000ff"

run_test "Width - force 64-bit" \
    "$BITZ -n --width=64 0xff" \
    "0x00000000000000ff"

run_test "Width - force 128-bit" \
    "$BITZ -n --width=128 0xff" \
...
run_test "Compact - multiple values" \
    "$BITZ -n --compact 0xff 0xaa 0x55 | wc -l | tr -d ' '" \
    "3"
...
run_test "Format - decimal only" \
    "$BITZ -n '--format=%d' 0xff" \
    "255"

run_test "Format - signed" \
    "$BITZ -n '--format=%s' 0xffffffff" \
    "-1"

run_test "Format - popcount" \
    "$BITZ -n '--format=pop=%p' 0xff" \
    "pop=8"

run_test "Format - highest bit" \
    "$BITZ -n '--format=hi=%hi' 0xff" \
    "hi=7"

run_test "Format - lowest bit" \
    "$BITZ -n '--format=lo=%lo' 0xf0" \
    "lo=4"

run_test "Format - leading zeros" \
    "$BITZ -n '--format=lz=%lz' 0xff" \
    "lz=24"

run_test "Format - trailing zeros" \
    "$BITZ -n '--format=tz=%tz' 0xf0" \
    "tz=4"

run_test "Format - width" \
    "$BITZ -n '--format=width=%w' 0xff" \
    "width=32"
...
run_test "Format - multiple values" \
    "$BITZ -n '--format=%h' 0xff 0xaa 0x55 | wc -l | tr -d ' '" \
    "3"
