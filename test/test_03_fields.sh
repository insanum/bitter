...
#
# EXAMPLES:
#   bitter --def="[7:0]=DATA" 0x1234         # Define DATA field as bits 7:0
#   bitter --def="[15:8]=STATUS,[7:0]=CMD" 0x1234  # Multiple fields
#   bitter --def="[0]=ENABLE" 0x01           # Single-bit field
#   bitter --regfile=regs.def 0x1234         # Load fields from file
#
...
run_test "Define single field" \
    "$BITTER -n '--def=[7:0]=DATA' 0xff" \
    "DATA"

run_test "Field shows extracted value (hex)" \
    "$BITTER -n '--def=[7:0]=DATA' 0x12" \
    "0x12"
...
run_test "Field extraction - upper byte" \
    "$BITTER -n '--def=[15:8]=STATUS' 0x1234" \
    "0x12"

run_test "Field extraction - middle bits" \
    "$BITTER -n '--def=[11:4]=MID' 0xabc" \
    "0xab"
...
run_test "Single-bit field (set)" \
    "$BITTER -n '--def=[0]=ENABLE' 0x01" \
    "SET"

run_test "Single-bit field (clear)" \
    "$BITTER -n '--def=[0]=ENABLE' 0x00" \
    "CLEAR"

run_test "Single-bit field - high bit" \
    "$BITTER -n '--def=[31]=BUSY' 0x80000000" \
    "SET"
...
run_test "Multiple fields - comma separated" \
    "$BITTER -n '--def=[15:8]=STATUS,[7:0]=DATA' 0x1234" \
    "STATUS"
...
run_test "Multiple fields - correct STATUS value" \
    "$BITTER -n '--def=[15:8]=STATUS,[7:0]=DATA' 0x1234 | grep STATUS" \
    "0x12"

run_test "Multiple fields - correct DATA value" \
    "$BITTER -n '--def=[15:8]=STATUS,[7:0]=DATA' 0x1234 | grep DATA" \
    "0x34"
...
run_test "Complex fields - 4 fields" \
    "$BITTER -n '--def=[31:24]=A,[23:16]=B,[15:8]=C,[7:0]=D' 0xdeadbeef | grep -c '='" \
    "4"

run_test "Complex fields - field A value" \
    "$BITTER -n '--def=[31:24]=A,[23:16]=B,[15:8]=C,[7:0]=D' 0xdeadbeef | grep 'A '" \
    "0xde"

run_test "Complex fields - field D value" \
    "$BITTER -n '--def=[31:24]=A,[23:16]=B,[15:8]=C,[7:0]=D' 0xdeadbeef | grep 'D '" \
    "0xef"
...
TEMP_REGFILE="/tmp/bitter_test_regs.def"
...
run_test "Load regfile - DEVICE_ID field" \
    "$BITTER -n --regfile=$TEMP_REGFILE 0xdead1234" \
    "DEVICE_ID"
...
run_test "Load regfile - DEVICE_ID value" \
    "$BITTER -n --regfile=$TEMP_REGFILE 0xdead1234 | grep DEVICE_ID" \
    "0xdead"

run_test "Load regfile - VENDOR_ID value" \
    "$BITTER -n --regfile=$TEMP_REGFILE 0xdead1234 | grep VENDOR_ID" \
    "0x1234"

run_test "Load regfile - single bit field" \
    "$BITTER -n --regfile=$TEMP_REGFILE 0x80000001 | grep BUSY" \
    "SET"

run_test "Load regfile - ENABLE field" \
    "$BITTER -n --regfile=$TEMP_REGFILE 0x80000001 | grep ENABLE" \
    "SET"
...
run_test "Fields with DIFF - shows CHANGED" \
    "$BITTER -n '--def=[7:0]=DATA' --diff 0xff 0xaa" \
    "CHANGED"
...
run_test "Fields with NOT" \
    "$BITTER -n '--def=[7:0]=DATA' --not 0x0f | grep DATA" \
    "0xf0"
...
run_test "Fields with OR" \
    "$BITTER -n '--def=[7:0]=DATA' --or 0xf0 0x0f | grep 'DATA.*0xff'" \
    "0xff"
...
run_test "Field spanning full 32 bits" \
    "$BITTER -n '--def=[31:0]=FULL' 0xdeadbeef | grep FULL" \
    "0xdeadbeef"
...
run_test "Adjacent fields - no gap" \
    "$BITTER -n '--def=[7:4]=HIGH,[3:0]=LOW' 0xab | grep HIGH" \
    "0xa"

run_test "Adjacent fields - LOW value" \
    "$BITTER -n '--def=[7:4]=HIGH,[3:0]=LOW' 0xab | grep LOW" \
    "0xb"
...
run_test "Fields with gap" \
    "$BITTER -n '--def=[15:12]=A,[3:0]=B' 0xf00f | grep -c '='" \
    "2"
...
run_test "Reversed bit order" \
    "$BITTER -n '--def=[0:7]=DATA' 0xff | grep DATA" \
    "0xff"
